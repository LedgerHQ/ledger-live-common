 ...
 */
const formatCounterValueDay = (d: Date) =>
...
function createCounterValues<State>({
  ...
  const rateByDaySelector = createSelector(
    ...
    (rates, date) =>
      (rates &&
        ((date && rates[formatCounterValueDay(date)]) || rates.latest)) ||
      0
  );
  ...
  const calculateSelector = createSelector(
    ...
    (rate, value, disableRounding) =>
      disableRounding ? value * rate : Math.round(value * rate)
  );
  ...
  const poll: Poll = () => async (dispatch, getState) => {
    ...
    const pairs = userPairs.map(p => {
      ...
      if (p.exchange) {
        ...
        if (!histodays) return pair;
        const keys = Object.keys(histodays).sort((a, b) => (a < b ? 1 : -1));
        if (keys[0] === "latest") return pair;
        pair.afterDay = keys[0];
      }
      return pair;
    });
    if (pairs.length === 0) return;
    const { data } = await axios.post(getAPIBaseURL() + "/rates/daily", {
      pairs
    });
    // TODO validate data
    if (data) {
      ...
      if (unsetExchangesPairs.length > 0) {
        dispatch(setPairUnsetExchangesAction(unsetExchangesPairs));
      }
    }
  };
  ...
  // TODO we need to store latest somewhere else because can help perfs: latest will get updated much more often that the days.
    ...
    poll = throttle(() => {
      ...
      return new Promise(success => {
        this.setState(prevState => {
          ...
          this.props
            .poll()
            // TODO poll() should have a timeout, because we don't want this promise to hang forever
            .then(() => {
