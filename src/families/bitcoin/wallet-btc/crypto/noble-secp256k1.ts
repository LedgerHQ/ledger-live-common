...
class JacobianPoint {
  ...
  equals(other: JacobianPoint) {
    ...
    return (
      mod(this.x.multiply(bz2)).eq(mod(az2.multiply(other.x))) &&
      mod(this.y.multiply(bz3)).eq(mod(az3.multiply(other.y)))
    );
  }
  negate() {
  ...
  double() {
    ...
    const D = mod(
      _2n.multiply(
        mod(mod(X1.add(B).multiply(X1.add(B))))
          .subtract(A)
          .subtract(C)
      )
    );
  ...
  multiply(scalar, affinePoint) {
    ...
    const [point, fake] = [k1p.add(k2p), f1p.add(f2p)];
...
export function publicKeyTweakAdd(
  ...
  return Point.fromCompressedHex(publicKey)
    .add(Point.fromPrivateKey(tweak))
