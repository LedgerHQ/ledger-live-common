...
const getExtra = (type, extrinsic) => {
    let extra: {
        ...
        palletMethod: extrinsic.call_module
            ? `${extrinsic.call_module}.${camelCase(
            ...
            extra = {
                ...
                validators: params
                    ...
                    .value.reduce((old, current) => {
                        return current
                            ? [...old, encodePolkadotAddr(current)]
                            : old
                    }, []),
            }
            break
...
const subscanAmountToPlanck = (amount, blockHeight) => {
    ...
    return new BigNumber(amount).multipliedBy(100).multipliedBy(SUBSCAN_MULTIPLIER)
}
...
const fetchSubscanList = async (
    ...
    if (prevOperations.length) {
        const oldestBlockHeight =
            (prevOperations[prevOperations.length - 1] as { blockHeight: number }).blockHeight

        if (oldestBlockHeight < startAt) {
            return prevOperations.filter(o => (o as { blockHeight: number }).blockHeight >= startAt)
        }
    }
