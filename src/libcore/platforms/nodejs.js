...
export default (arg: {
  ...
  const loadCore = (): Promise<Core> => {
    ...
    function wrapResult(id, arg) {
      if (!arg || !id) return arg;
      if (Array.isArray(id)) {
        const [actualId] = id;
        return arg.map(arg => wrapResult(actualId, arg));
      }
      if (id in unwrappers) {
        return unwrappers[id](arg);
      }
      const Clz = mappings[id];
      if (!Clz) {
        return arg;
      }
      if (arg instanceof Clz) return arg;
      return new Clz(arg);
    }

    function unwrapArg(id, arg) {
      if (!arg || !id) return arg;
      if (Array.isArray(id)) {
        const [actualId] = id;
        return arg.map(arg => unwrapArg(actualId, arg));
      }
      if (id in wrappers) {
        return wrappers[id](arg);
      }
      return arg;
    }
      ...
      if (methods) {
        Object.keys(methods).forEach(method => {
          ...
          if (njsField) {
            ...
            m.prototype[method] = async function() {
              const args = params
                ? [...arguments].map((value, i) => unwrapArg(params[i], value))
                : arguments;
    ...
    mappings.EventBus.prototype.subscribe = function(
      executionContext,
      receiver
    ) {
